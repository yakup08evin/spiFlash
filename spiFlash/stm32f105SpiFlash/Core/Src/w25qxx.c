...
void W25qxx_Write(uint8_t buffer[], uint32_t NumByteToWrite){
	uint8_t temproryBuffer[NumByteToWrite];
	if(spistruct.sectorNumberforWrite >= 4094){
		spistruct.sectorNumberforWrite = 4094;
		spistruct.writeOffset = 0;
	}
	W25qxx_WriteSector(buffer,spistruct.sectorNumberforWrite,spistruct.writeOffset,NumByteToWrite);
	spistruct.writeOffset = spistruct.writeOffset + NumByteToWrite;
	printmsg("---%ld\r\n",spistruct.writeOffset);
	if(spistruct.writeOffset >= 4096){
		spistruct.sectorNumberforWrite++;
		spistruct.bufferCountStoreforWrite = NumByteToWrite;
		NumByteToWrite = ((spistruct.writeOffset)-4096);
		spistruct.writeOffset = 0;
		if(NumByteToWrite != 0){
			memset(temproryBuffer,0,sizeof(temproryBuffer));
			memcpy(temproryBuffer,&buffer[spistruct.bufferCountStoreforWrite-NumByteToWrite],NumByteToWrite*sizeof(uint8_t));
			W25qxx_WriteSector(temproryBuffer,spistruct.sectorNumberforWrite,spistruct.writeOffset,NumByteToWrite);
		}
		spistruct.writeOffset = spistruct.writeOffset + NumByteToWrite;
	}
